...
// 		(files: FileWithPreview[], base64Images: string[]) => {
...
// 		},
// 		[setValue] // Add other dependencies if needed
// 	);

// 	// Fixed useEffect to prevent infinite loops
// 	React.useEffect(() => {
// 		if (!onSale || basePrice == null || isNaN(Number(basePrice))) return;

// 		const base = parseFloat(basePrice.toString() || "0");

// 		// If discount is being edited
// 		if (discount != null && !isNaN(Number(discount))) {
// 			const disc = parseFloat(discount.toString() || "0");
// 			const calculatedSalePrice = (base * (1 - disc / 100)).toFixed(2);
// 			setValue("salePrice", parseFloat(calculatedSalePrice), {
// 				shouldDirty: true,
// 			});
// 		}
// 		// If salePrice is being edited
// 		else if (salePrice != null && !isNaN(Number(salePrice))) {
// 			const sale = parseFloat(salePrice.toString() || "0");
// 			if (base > 0) {
// 				const calculatedDiscount = (((base - sale) / base) * 100).toFixed(2);
// 				setValue("discount", parseFloat(calculatedDiscount), {
// 					shouldDirty: true,
// 				});
// 			}
// 		}
// 	}, [onSale, basePrice, salePrice, discount, setValue]);
...
import { Badge } from "@/components/ui/badge";
import { Separator } from "@/components/ui/separator";
import UploadImageComponent from "@/components/Seller/sell-product/UploadImageComponent";
import { FileWithPreview } from "@/hooks/use-file-upload";
import { Plus, X, DollarSign, Tag, Box, Loader2, Edit3 } from "lucide-react";
import { createProductSchema } from "@/schemas/product.validation";
import { Checkbox } from "@/components/ui/checkbox";
		...
		const loadProduct = async () => {
			...
			try {
				...
				if (product) {
					...
					// Fix: Handle category array with proper type casting
					...
					setValue("category", processedCategories);
					...
					setValue("saleTax", product.saleTax);

					// Handle keyFeatures array
					if (Array.isArray(product.keyFeatures)) {
						setValue(
							"keyFeatures",
							product.keyFeatures.map((f) => ({ value: f }))
						);
					} else {
						setValue("keyFeatures", [{ value: "This is the feature 1." }]);
					}
					...
					if (Array.isArray(product.variants)) {
						...
						setValue("variants", [{ size: "", color: "", stock: 1 }]);
					}
				...
				router.push("/seller/dashboard/product/manage");
			} finally {
				setLoading(false);
			}
		};
			...
			<Card className="h-fit">
				<CardHeader className="flex flex-row items-center justify-between py-4">
					<CardTitle className="flex items-center gap-2">
						{isEditMode ? <Edit3 className="h-5 w-5" /> : null}
								...
								{isEditMode && (
									<div className="bg-blue-50 border border-blue-200 rounded-md p-3 text-sm">
										...
										<p className="text-blue-700">
											If you want to change images, you need to re-upload all
											images again. Current images will be replaced with the new
											ones.
										</p>
									</div>
								)}
						...
						<Button type="submit" className="w-full" disabled={isSubmitting}>
							{isSubmitting ? (
								...
								<>
									...
									{isEditMode ? "Update Product" : "Save Product Information"}
								</>
							)}
						</Button>
					</form>
