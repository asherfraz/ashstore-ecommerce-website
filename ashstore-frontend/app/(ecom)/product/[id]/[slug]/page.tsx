...
// 											// implement like / removeLike on click functionality
// 											// onClick={}
// 										>
...
// 											size="sm"
// 											className="flex items-center gap-1"
// 											// implement dislike / removeDislike on click functionality
// 											// onClick={}
// 										>
...
import {
	...
	addReviewReply,
	getProductById,
	updateProductReviewLikeDislike,
} from "@/api/productApis";
import { useDispatch } from "react-redux";
import { updateProduct } from "@/redux/productSlice";
import { useAuth } from "@/hooks/useAuth";
import { getUserProductReactions } from "@/api/userApis";
...
export default function ProductDetailPage() {
	...
	// NEW: Fetch user's reactions for this product
	useEffect(() => {
		const fetchUserReactions = async () => {
			if (user && productId) {
				try {
					const response = await getUserProductReactions(productId as string);
					const userReactionsData = response.data.reactions;

					// Update user reactions state with the fetched data
					const reactionsMap: Record<string, "like" | "dislike" | null> = {};

					if (userReactionsData && Array.isArray(userReactionsData)) {
						userReactionsData.forEach((reaction: any) => {
							// reaction.reviewId is the review ID
							// reaction.reaction is the user's reaction ("like", "dislike", or null)
							reactionsMap[reaction.reviewId] = reaction.reaction;
						});
					}

					setUserReactions((prev) => ({
						...prev, // Keep any existing reactions
						...reactionsMap, // Override with fetched reactions
					}));
				} catch (error) {
					console.error("Error fetching user reactions:", error);
					// Don't update state on error, keep existing reactions
				}
			}
		};

		fetchUserReactions();
	}, [user, productId]); // Only run when user or productId changes
	...
	useEffect(() => {
		...
		const fetchProductById = async () => {
			try {
				...
				if (response?.data?.success) {
					...
					// Initialize user reactions from the reviews
					const initialReactions: Record<string, "like" | "dislike" | null> =
						{};
					productData.reviews.forEach((review) => {
						// Initialize to null since we don't have user-specific reaction data yet
						// The actual reactions will be loaded by the fetchUserReactions useEffect
						initialReactions[review._id!] = null;
					});
		...
		if (filterProduct) {
			...
			const initialReactions: Record<string, "like" | "dislike" | null> = {};
			filterProduct.reviews.forEach((review) => {
				initialReactions[review._id!] = null;
			});
			setUserReactions(initialReactions);
	...
	}, [productId, reduxProducts.products, router]);
	...
	const handlePostReview = async () => {
		...
		try {
			...
			if (response?.data?.success) {
				...
				// Add new review to user reactions
				setUserReactions((prev) => ({
					...prev,
					[updatedProduct.reviews[updatedProduct.reviews.length - 1]._id!]:
						null,
				}));
	...
	const handleLikeDislike = async (
		...
		if (!user) {
			toast.error("Please login to like or dislike reviews");
			return;
		}

		if (!productId) return;

		setLoadingReactions((prev) => ({ ...prev, [reviewId]: true }));

		try {
			const response = await updateProductReviewLikeDislike(
				productId as string,
				reviewId,
				action
			);
			...
			if (response?.data?.success) {
				...
				setUserReactions((prev) => {
					...
					if (action === "like") {
						newReactions[reviewId] = "like";
					} else if (action === "dislike") {
						newReactions[reviewId] = "dislike";
					} else if (action === "removeLike" || action === "removeDislike") {
						newReactions[reviewId] = null;
					}
					return newReactions;
				});
				...
				const actionText = action.replace("remove", "").toLowerCase();
				toast.success(`Review ${actionText} updated successfully!`);
			} else {
				toast.error(
					response?.data?.message ||
						"Failed to update review reaction. Please try again."
				);
			}
		} catch (error: any) {
			...
			setLoadingReactions((prev) => ({ ...prev, [reviewId]: false }));
		}
	};

	// Check if current user has liked/disliked a review
	const checkUserReaction = (reviewId: string) => {
		...
		const reaction = userReactions[reviewId] || null;
		...
		product && (
			<div className="container mx-auto px-4 py-8 space-y-12">
				...
				<section>
					<Card>
						<CardContent>
							<div className="flex items-center gap-6">
								<div className="text-center">
									<p className="text-4xl font-bold">{product.averageRating}</p>
									<div className="flex justify-center text-yellow-500">
										{Array.from({
											length: Math.round(product.averageRating || 0),
										}).map((_, i) => (
				...
				<section className="mt-16">
					...
					<div className="space-y-6">
						{visibleReviews.map((review) => {
							const userReaction = checkUserReaction(review._id!);
							const isLoading = loadingReactions[review._id || ""];
									...
									<CardContent>
										...
										<div className="flex items-center gap-4 text-sm text-muted-foreground mb-2">
											<Button
												...
												size="sm"
												className={`flex items-center gap-1 ${
													userReaction.hasLiked ? "text-blue-500" : ""
												}`}
												onClick={() => {
													if (!user) {
													...
													// Toggle between like and removeLike
													const action = userReaction.hasLiked
														? "removeLike"
														: "like";
													handleLikeDislike(review._id!, action);
												}}
												disabled={isLoading}
											>
												<ThumbsUp
											...
											<Button
												...
												size="sm"
												className={`flex items-center gap-1 ${
													userReaction.hasDisliked ? "text-red-500" : ""
												}`}
												onClick={() => {
													if (!user) {
													...
													// Toggle between dislike and removeDislike
													const action = userReaction.hasDisliked
														? "removeDislike"
														: "dislike";
													handleLikeDislike(review._id!, action);
												}}
